...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("#6d1d76").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Lupin"}& Witcher`).setTimestamp();
  ...
  if (command === "kur" || command === "kurulum" || command === "setup") {
    ...
    Database.findOne({guildID: ayarlar.guildID, roleID: args[0]}, async (err, roleData) => {
      ...
      if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('DmForBan').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`Lupin`).setTimestamp()).catch(); } else { message.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Lupin"}& Witcher`).setTimestamp()).catch(err => {}); };
    });
  };
});
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "DmForBan" }).catch();
};
...
function setRoleBackup() {
  ...
  if (guild) {
    ...
    console.log(`DmForBan Rol Yedeği Alındı! ${moment(Date.now()).format("DD")} ${aylar[moment(Date.now()).format("MM")]} ${moment(Date.now()).format("YYYY HH:mm:ss")} `);
  };
};
